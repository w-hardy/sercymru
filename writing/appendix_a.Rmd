...
glsData <- function(data){
  ...
  baseline <- subset(df, Month == 2, -Month)
  baseline <- upData(baseline, rename = c(pct_change = "feb_diff"), print = FALSE)
  followup <- subset(df, Month > 2)
...
redun <- list()
for(i in drugs){
  redun[[i]] <- redun(~hboard + dispensing + Month + feb_diff + wimd2019 + 
                        PC1 + PC2 + PC3 + PC4 + PC5 + ngp + gp_load,
                      data = glsData(i), r2 = .8, type = "adjusted") %>% 
    .$Out
}

# No redundent variables
...
```{r, gls-analysis-save, results="asis", fig.cap=gls_analysis_fig_cap, warning=FALSE, message=FALSE, eval = FALSE}
#### gls-analysis-save ####
drugs <- unique(differences$drug)
