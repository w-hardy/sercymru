...
## WIMD
drugs_wimd <- c("cortico", "sal", "ssri", "vitd")
...
for(i in drugs){
  ...
  gls_month_fig[[i]] <- 
    Predict(best_bs_models[[i]], Month, 
            ...
            gp_load = 1329.486, 
            PC1 = -0.1257104, PC2 = 0.1257104, PC3 = 0.1257104, PC4 = -0.09966945,
            conf.int = .95)
...
disp_contrasts <- 
  list(
    all_drugs = contrast(best_bs_models$all_drugs, 
                         list(Month = 3:8, dispensing = TRUE),
                         list(Month = 3:8, dispensing = FALSE)) %>% 
...
ggsave(filename = "Draft 5/Figures/dispensing-status.png", device = "png", height = 14, width = 25, units = "cm")
...
ngp_contrasts <- 
  list(
    oral_contra = contrast(best_bs_models$oral_contra, 
                       list(Month = 3:8, ngp = 3),
                       list(Month = 3:8, ngp = 8)) %>% 
      .[c("Month", "Contrast", "Lower", "Upper")] %>% 
      as.data.frame(),
    sal = contrast(best_bs_models$sal, 
                   list(Month = 3:8, ngp = 3),
...
gpld_contrasts <- 
  list(
    oral_contra = contrast(best_bs_models$oral_contra, 
                       list(Month = 3:8, gp_load = 1098),
                       list(Month = 3:8, gp_load = 1710)) %>% 
      .[c("Month", "Contrast", "Lower", "Upper")] %>% 
      as.data.frame()) %>% 
  bind_rows(.id = "drug")
