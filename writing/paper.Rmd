...
glsAssump <- function(model, data){
  ...
  p2 <- ggplot(data, aes(x = Month, y = resid)) + y1 + 
    stat_summary(fun.data = "mean_sdl", geom = "smooth")
  p3 <- ggplot(data, aes(sample = resid)) + stat_qq(alpha = .1) + 
...
drugs <- factor(drugs)
drugs <- relevel(drugs, ref = "all_drugs")
```
...
tibble(regional_unit = unique(differences$regional_unit)) %>% 
  ...
  mutate(across(p_male:flu, ~.x*100),
         dispensing = if_else(dispensing == "Yes", 1, 0)) %>% 
  tibble() %>% 
...
pres_2019 %>% 
  ...
  bind_rows(read_rds(data_files["warfarin_ddd"]) %>% 
...
differences_quantiles %>%
  ...
  dplyr::mutate(drug = case_when(drug == "all_drugs" ~ "All dispensing",
                          drug == "ace" ~ "ACEi",
         ...
         drug = relevel(as_factor(drug), ref = "All dispensing"),
...
disp_contrasts <- 
  list(
    all_drugs = contrast(best_bs_models$all_drugs, 
                         list(Month = 3:8, wimd2019 = "Med", 
                              ngp = median(gp_2020$ngp), 
                              gp_load = median(gp_2020$gp_load),
...
disp_contrasts %>% 
  ...
  mutate(across(where(is.numeric), printnum),
         contrast = paste0(Contrast, "[", Lower, ",", Upper, "]"),
         Month = factor(x = Month, levels = 3:8,
                           labels = month.abb[3:8])) %>%
  pivot_wider(id_cols = drug, values_from = contrast, names_from = Month) %>%
  kable(align = "r") %>%
  kable_styling() %>%
  landscape()
```
...
wimd_contrasts <- 
  list(
    ace = contrast(best_bs_models$ace, 
                   list(Month = 3:8, wimd2019 = "Low", 
                        ngp = median(gp_2020$ngp), 
                        gp_load = median(gp_2020$gp_load),
                        PC1 = median(gp_2020$PC1), PC2 = median(gp_2020$PC2), 
                        PC3 = median(gp_2020$PC3), PC4 = median(gp_2020$PC4),
                        dispensing = FALSE),
                   list(Month = 3:8, wimd2019 = "High", 
                        ngp = median(gp_2020$ngp), 
                        gp_load = median(gp_2020$gp_load),
                        PC1 = median(gp_2020$PC1), PC2 = median(gp_2020$PC2), 
                        PC3 = median(gp_2020$PC3), PC4 = median(gp_2020$PC4),
...
contrasts <- 
  ...
  mutate(Month = factor(x = Month, levels = 3:8, 
         ...
         drug = case_when(drug == "all_drugs" ~ "All dispensing",
                          drug == "ace" ~ "ACEi",
...
contrasts %>% 
  ...
  labs(col = "Drug") +
  facet_wrap(~effect) +
  labs(caption = paste0("Unless specified, predictors adjusted to Dispensing status = FALSE, WIMD = Med, Number GPs = ", printnum(median(gp_2020$ngp)),", \nGP workload = ", printnum(median(gp_2020$gp_load)),", PC1 = ", printnum(median(gp_2020$PC1)),", PC2 = ", printnum(median(gp_2020$PC1)), ", PC3 = ", printnum(median(gp_2020$PC3)), ", PC4 = ", printnum(median(gp_2020$PC4))))
