...
nsaid <- 
  read_rds("../data/presdata_updates_040121/presc_wales_NSAIDs.rds") %>% 
  janitor::clean_names() %>% 
...
nsaid_monthly_quantity_gp <- 
  countRegional(nsaid, practice_id, datename, quantity) %>% 
...
nsaid_monthly_count_pre_cv19 <- 
  nsaid_monthly_count %>% 
  filter(datename < yearmonth(dmy("01-03-2020"))) 

nsaid_monthly_quantity_gp_pre_cv19 <- 
  nsaid_monthly_quantity_gp %>% 
  filter(datename < yearmonth(dmy("01-03-2020"))) 
...
nsaid_monthly_count_pre_cv19 <- 
  nsaid_monthly_count_pre_cv19 %>% 
  model(mean = MEAN(n)) %>%
  interpolate(nsaid_monthly_count_pre_cv19)

nsaid_monthly_quantity_gp_pre_cv19 <- 
  nsaid_monthly_quantity_gp_pre_cv19%>% 
  group_by(regional_unit) %>% 
  model(mean = MEAN(n)) %>% 
  interpolate(nsaid_monthly_quantity_gp_pre_cv19)
...
```{r, nsaid-gp-practice-modelling}
plan(cluster)
system.time(nsaid_gp_cv_models <-
  nsaid_monthly_quantity_gp_pre_cv19 %>%
                  regionalCV(init = 48, step = 3))
plan(sequential)

nsaid_gp_cv_models %>%
  saveRDS("../data/nsaid_gp_cv_models.rds")

nsaid_gp_cv_models <- 
  read_rds("../data/nsaid_gp_cv_models.rds")
...
```{r nsaid-gp-practice-forecasts}
plan(cluster)
system.time(nsaid_gp_fcsts <-
              nsaid_monthly_quantity_gp_pre_cv19 %>%
...
nsaid_gp_fcsts %>%
  saveRDS("../data/nsaid_gp_fcsts.rds")

nsaid_gp_fcsts <- 
  read_rds("../data/nsaid_gp_fcsts.rds")
...
```{r, nsaid-gp-practice-discrepancies}
best_model_gp <- 
  nsaid_gp_cv_models %>% 
  group_by(regional_unit) %>% 
  arrange(RMSE, MAE) %>% 
...
best_model_gp_discrep <- 
  ...
  inner_join(mutate(nsaid_gp_fcsts,
                    datename = yearmonth(datename)), 
             by = c(".model", "regional_unit")) %>%
  left_join(mutate(select(nsaid_monthly_quantity_gp, 
...
best_model_gp_discrep %>% 
  saveRDS("../data/differences/nsaid_discreps.rds")
