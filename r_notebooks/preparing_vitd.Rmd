---
title: "Respiratory vitdsteroids"
output: 
  html_document:
    code_folding: hide
    toc: true
...
vitd <- 
  read_rds("../data/presdata_updates_040121/presc_wales_vitamind.rds") %>% 
  janitor::clean_names() %>% 
...
vitd_monthly_quantity_gp <- 
  countRegional(vitd, practice_id, datename, quantity) %>% 
...
vitd_monthly_count_pre_cv19 <- 
  vitd_monthly_count %>% 
  filter(datename < yearmonth(dmy("01-03-2020"))) 

vitd_monthly_quantity_gp_pre_cv19 <- 
  vitd_monthly_quantity_gp %>% 
  filter(datename < yearmonth(dmy("01-03-2020"))) 
...
vitd_monthly_count_pre_cv19 <- 
  vitd_monthly_count_pre_cv19 %>% 
  model(mean = MEAN(n)) %>%
  interpolate(vitd_monthly_count_pre_cv19)

vitd_monthly_quantity_gp_pre_cv19 <- 
  vitd_monthly_quantity_gp_pre_cv19%>% 
  group_by(regional_unit) %>% 
  model(mean = MEAN(n)) %>% 
  interpolate(vitd_monthly_quantity_gp_pre_cv19)
...
```{r, vitd-gp-practice-modelling}
plan(cluster)
system.time(vitd_gp_cv_models <-
  vitd_monthly_quantity_gp_pre_cv19 %>%
                  regionalCV(init = 48, step = 3))
plan(sequential)

vitd_gp_cv_models %>%
  saveRDS("../data/vitd_gp_cv_models.rds")

vitd_gp_cv_models <- 
  read_rds("../data/vitd_gp_cv_models.rds")
...
```{r vitd-gp-practice-forecasts}
plan(cluster)
system.time(vitd_gp_fcsts <-
              vitd_monthly_quantity_gp_pre_cv19 %>%
...
vitd_gp_fcsts %>%
  saveRDS("../data/vitd_gp_fcsts.rds")

vitd_gp_fcsts <- 
  read_rds("../data/vitd_gp_fcsts.rds")
...
```{r, vitd-gp-practice-discrepancies}
best_model_gp <- 
  vitd_gp_cv_models %>% 
  group_by(regional_unit) %>% 
  arrange(RMSE, MAE) %>% 
...
best_model_gp_discrep <- 
  ...
  inner_join(mutate(vitd_gp_fcsts,
                    datename = yearmonth(datename)), 
             by = c(".model", "regional_unit")) %>%
  left_join(mutate(select(vitd_monthly_quantity_gp, 
...
best_model_gp_discrep %>% 
  saveRDS("../data/differences/vitd_discreps.rds")
