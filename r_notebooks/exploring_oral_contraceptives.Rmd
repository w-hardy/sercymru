...
best_model_hb %>% 
  ...
  inner_join(mutate(oc_hb_fcsts$forecasts,
...
best_model_gp_cluster %>% 
  select(.model, regional_unit) %>% 
  inner_join(mutate(oc_gp_cluster_fcsts$forecasts,
                    datename = yearmonth(datename)), 
             by = c(".model", "regional_unit")) %>%
...
best_model_gp %>% 
  select(.model, regional_unit) %>% 
  inner_join(mutate(oc_gp_fcsts$forecasts,
                    datename = yearmonth(datename)), 
             by = c(".model", "regional_unit")) %>%
...
bestModelPlot(models = oc_gp_cluster_cv_models, 
              forecasts = oc_gp_cluster_fcsts$forecasts,
