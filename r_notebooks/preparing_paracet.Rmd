---
title: "Respiratory paracetsteroids"
output: 
  html_document:
    code_folding: hide
    toc: true
...
paracet <- 
  read_rds("../data/presdata_updates_040121/presc_wales_paracetamol.rds") %>% 
  janitor::clean_names() %>% 
  mutate(datename = dmy(paste0("28-", datename)),
         practice_id = as_factor(practiceid))

## All data sorted into monthly summary
paracet_monthly_count <- 
  paracet %>% 
  group_by(datename) %>% 
...
paracet_monthly_quantity_gp <- 
  countRegional(paracet, practice_id, datename, quantity) %>% 
...
paracet_monthly_count_pre_cv19 <- 
  paracet_monthly_count %>% 
  filter(datename < yearmonth(dmy("01-03-2020"))) 

paracet_monthly_quantity_gp_pre_cv19 <- 
  paracet_monthly_quantity_gp %>% 
  filter(datename < yearmonth(dmy("01-03-2020"))) 
...
paracet_monthly_count_pre_cv19 <- 
  paracet_monthly_count_pre_cv19 %>% 
  model(mean = MEAN(n)) %>%
  interpolate(paracet_monthly_count_pre_cv19)

paracet_monthly_quantity_gp_pre_cv19 <- 
  paracet_monthly_quantity_gp_pre_cv19%>% 
  group_by(regional_unit) %>% 
  model(mean = MEAN(n)) %>% 
  interpolate(paracet_monthly_quantity_gp_pre_cv19)
...
```{r, paracet-gp-practice-modelling}
plan(cluster)
system.time(paracet_gp_cv_models <-
  paracet_monthly_quantity_gp_pre_cv19 %>%
                  regionalCV(init = 48, step = 3))
plan(sequential)

paracet_gp_cv_models %>%
  saveRDS("../data/paracet_gp_cv_models.rds")

paracet_gp_cv_models <- 
  read_rds("../data/paracet_gp_cv_models.rds")
...
```{r paracet-gp-modelling-accuracy}
best_model_gp <- 
  paracet_gp_cv_models %>% 
  group_by(regional_unit) %>% 
  arrange(RMSE, MAE) %>% 
...
```{r paracet-gp-practice-forecasts}
plan(cluster)
system.time(paracet_gp_fcsts <-
              paracet_monthly_quantity_gp_pre_cv19 %>%
...
paracet_gp_fcsts %>%
  saveRDS("../data/paracet_gp_fcsts.rds")

paracet_gp_fcsts <- 
  read_rds("../data/paracet_gp_fcsts.rds")
...
```{r, paracet-gp-practice-discrepancies}
best_model_gp <- 
  paracet_gp_cv_models %>% 
  group_by(regional_unit) %>% 
  arrange(RMSE, MAE) %>% 
...
best_model_gp_discrep <- 
  ...
  inner_join(mutate(paracet_gp_fcsts,
                    datename = yearmonth(datename)), 
             by = c(".model", "regional_unit")) %>%
  left_join(mutate(select(paracet_monthly_quantity_gp, 
...
best_model_gp_discrep %>% 
  saveRDS("../data/differences/paracet_discreps.rds")
