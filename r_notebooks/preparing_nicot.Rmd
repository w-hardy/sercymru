---
title: "Respiratory nicotsteroids"
output: 
  html_document:
    code_folding: hide
    toc: true
...
nicot <- 
  read_rds("../data/presdata_updates_040121/presc_wales_nicotine.rds") %>% 
  janitor::clean_names() %>% 
...
nicot_monthly_quantity_gp <- 
  countRegional(nicot, practice_id, datename, quantity) %>% 
...
nicot_monthly_count_pre_cv19 <- 
  nicot_monthly_count %>% 
  filter(datename < yearmonth(dmy("01-03-2020"))) 

nicot_monthly_quantity_gp_pre_cv19 <- 
  nicot_monthly_quantity_gp %>% 
  filter(datename < yearmonth(dmy("01-03-2020"))) 
...
nicot_monthly_count_pre_cv19 <- 
  nicot_monthly_count_pre_cv19 %>% 
  model(mean = MEAN(n)) %>%
  interpolate(nicot_monthly_count_pre_cv19)

nicot_monthly_quantity_gp_pre_cv19 <- 
  nicot_monthly_quantity_gp_pre_cv19%>% 
  group_by(regional_unit) %>% 
  model(mean = MEAN(n)) %>% 
  interpolate(nicot_monthly_quantity_gp_pre_cv19)
...
```{r, nicot-gp-practice-modelling}
plan(cluster)
system.time(nicot_gp_cv_models <-
  nicot_monthly_quantity_gp_pre_cv19 %>%
                  regionalCV(init = 48, step = 3))
plan(sequential)

nicot_gp_cv_models %>%
  saveRDS("../data/nicot_gp_cv_models.rds")

nicot_gp_cv_models <- 
  read_rds("../data/nicot_gp_cv_models.rds")
...
```{r nicot-gp-practice-forecasts}
plan(cluster)
system.time(nicot_gp_fcsts <-
              nicot_monthly_quantity_gp_pre_cv19 %>%
...
nicot_gp_fcsts %>%
  saveRDS("../data/nicot_gp_fcsts.rds")

nicot_gp_fcsts <- 
  read_rds("../data/nicot_gp_fcsts.rds")
...
```{r, nicot-gp-practice-discrepancies}
best_model_gp <- 
  nicot_gp_cv_models %>% 
  group_by(regional_unit) %>% 
  arrange(RMSE, MAE) %>% 
...
best_model_gp_discrep <- 
  ...
  inner_join(mutate(nicot_gp_fcsts,
                    datename = yearmonth(datename)), 
             by = c(".model", "regional_unit")) %>%
  left_join(mutate(select(nicot_monthly_quantity_gp, 
...
best_model_gp_discrep %>% 
  saveRDS("../data/differences/nicot_discreps.rds")
