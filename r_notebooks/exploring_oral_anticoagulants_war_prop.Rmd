...
oac_monthly_quantity_gp <- 
  propRegional(oac, PracticeID, datename, Quantity) %>% 
...
oac_gp_fcsts %>%
  write_rds("../data/oac_gp_fcsts_2.rds")

oac_gp_cluster_fcsts <- 
  read_rds("../data/oac_gp_fcsts_2.rds")
```
...
best_model_gp %>% 
  select(.model, regional_unit) %>% 
  inner_join(mutate(oac_gp_fcsts$forecasts,
                    datename = yearmonth(datename)), 
             by = c(".model", "regional_unit")) %>%
