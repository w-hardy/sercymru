...
hcq <- 
  read_rds("../data/presdata_updates_040121/presc_wales_HCQ.rds") %>% 
...
hcq_monthly_quantity_gp <- 
  countRegional(hcq, practice_id, datename, quantity) %>% 
...
hcq_monthly_count_pre_cv19 <- 
  hcq_monthly_count_pre_cv19 %>% 
  model(mean = MEAN(n)) %>%
  interpolate(hcq_monthly_count_pre_cv19)

hcq_monthly_quantity_gp_pre_cv19 <- 
  hcq_monthly_quantity_gp_pre_cv19%>% 
  group_by(regional_unit) %>% 
  model(mean = MEAN(n)) %>% 
  interpolate(hcq_monthly_quantity_gp_pre_cv19)
...
```{r, hcq-gp-practice-discrepancies}
best_model_gp <- 
  hcq_gp_cv_models %>% 
  group_by(regional_unit) %>% 
  arrange(RMSE, MAE) %>% 
...
best_model_gp_discrep %>% 
  saveRDS("../data/differences/hcq_discreps.rds")
