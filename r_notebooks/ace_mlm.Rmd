...
gp <- 
  ...
  mutate(npat = N,
         ...
         cluster = str_remove_all(cluster, "[/&]"),
...
* Removing gp_load as 0 effect and 0 variance

```{r, mar-mlm3-cluster-model}
mar_mlm3 <- brm(march ~ 1 + p_fem + pm_65 + dispensing + wimd2019 +
              (1 + p_fem + pm_65 + dispensing + wimd2019|cluster),  
      data = mlm_df, 
      warmup = 2000, iter = 6000, seed = 123, save_pars = save_pars(all = TRUE),
      control = list(max_treedepth = 15, adapt_delta = .999))

mar_mlm3_params <- modelParams(mar_mlm3)
```


```{r, mar-m3-cluster-convergence}
mar_mlm3 %>% 
  ggs() %>% 
  filter(Parameter %in% c("b_Intercept", "b_p_fem", "b_pm_65", "b_dispensingYes",
                          "b_wimd2019", "b_gp_load")) %>% 
...
apr_mlm1 <- brm(april ~ 1 + march +
              ...
              (1|cluster),  
      data = mlm_df, 
      warmup = 2000, iter = 4000, seed = 123, save_pars = save_pars(all = TRUE)) 
