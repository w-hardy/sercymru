...
system.time(proph_fit2 <- future_map_dfr(.x = unique(dev_data$regional_unit), 
             .f = ~regionalProphetFits(dev_data, .x),
            .options = furrr_options(seed = TRUE)))
...
```
1 error encountered for oc_prophet_model1
[1] unable to load shared object '/Library/Frameworks/R.framework/Versions/4.0/Resources/library/prophet/libs/prophet.so':
...
* 2020-12-08 Fixed
  * Reinstalling using `install.packages("prophet", type="source")` has resolved problem.
...
```{r}
cross_validation(select(oc_gp_cluster_fcts$fits, c(regional_unit, starts_with("oc_proph"))), horizon = , units = "years", initial = 3)
```
...
## parallel joint forecasts

Why does only one part of this run in parallel?
...
system.time(regionalJointFcsts(data = dev_data))
```


84.619 seconds when opening and closing plan(multisession) over 5 regional units vs 59.108 when leaving them open