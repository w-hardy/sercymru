...
azithro <- 
  read_rds("../data/presdata_updates_040121/presc_wales_azithromycin.rds") %>% 
  janitor::clean_names() %>% 
  mutate(datename = dmy(paste0("28-", datename)),
         practice_id = as_factor(practiceid))

## All data sorted into monthly summary
azithro_monthly_count <- 
  azithro %>% 
  group_by(datename) %>% 
...
azithro_monthly_quantity_gp <- 
  countRegional(azithro, practice_id, datename, quantity) %>% 
...
azithro_monthly_count_pre_cv19 <- 
  azithro_monthly_count %>% 
  filter(datename < yearmonth(dmy("01-03-2020"))) 

azithro_monthly_quantity_gp_pre_cv19 <- 
  azithro_monthly_quantity_gp %>% 
  filter(datename < yearmonth(dmy("01-03-2020"))) 
...
azithro_monthly_count_pre_cv19 <- 
  azithro_monthly_count_pre_cv19 %>% 
  model(mean = MEAN(n)) %>%
  interpolate(azithro_monthly_count_pre_cv19)

azithro_monthly_quantity_gp_pre_cv19 <- 
  azithro_monthly_quantity_gp_pre_cv19%>% 
  group_by(regional_unit) %>% 
  model(mean = MEAN(n)) %>% 
  interpolate(azithro_monthly_quantity_gp_pre_cv19)
...
```{r, azithro-gp-practice-modelling}
plan(cluster)
system.time(azithro_gp_cv_models <-
  azithro_monthly_quantity_gp_pre_cv19 %>%
                  regionalCV(init = 48, step = 3))
plan(sequential)

azithro_gp_cv_models %>%
  saveRDS("../data/azithro_gp_cv_models.rds")

azithro_gp_cv_models <- 
  read_rds("../data/azithro_gp_cv_models.rds")
...
```{r azithro-gp-modelling-accuracy}
best_model_gp <- 
  azithro_gp_cv_models %>% 
  group_by(regional_unit) %>% 
  arrange(RMSE, MAE) %>% 
...
```{r azithro-gp-practice-forecasts}
plan(cluster)
system.time(azithro_gp_fcsts <-
              azithro_monthly_quantity_gp_pre_cv19 %>%
...
azithro_gp_fcsts %>%
  saveRDS("../data/azithro_gp_fcsts.rds")

azithro_gp_fcsts <- 
  read_rds("../data/azithro_gp_fcsts.rds")
...
```{r, azithro-gp-practice-discrepancies}
best_model_gp <- 
  azithro_gp_cv_models %>% 
  group_by(regional_unit) %>% 
  arrange(RMSE, MAE) %>% 
...
best_model_gp_discrep <- 
  ...
  inner_join(mutate(azithro_gp_fcsts,
                    datename = yearmonth(datename)), 
             by = c(".model", "regional_unit")) %>%
  left_join(mutate(select(azithro_monthly_quantity_gp, 
...
best_model_gp_discrep %>% 
  saveRDS("../data/differences/azithro_discreps.rds")
