...
ace <- 
  read_rds("../data/presdata_updates_040121/presc_wales_ace-inhibitors.rds") %>% 
  janitor::clean_names() %>% 
...
ace_monthly_quantity_gp <- 
  countRegional(ace, practice_id, datename, quantity) %>% 
...
ace_monthly_count_pre_cv19 <- 
  ace_monthly_count %>% 
  filter(datename < yearmonth(dmy("01-03-2020"))) 

ace_monthly_quantity_gp_pre_cv19 <- 
  ace_monthly_quantity_gp %>% 
  filter(datename < yearmonth(dmy("01-03-2020"))) 
...
ace_monthly_count_pre_cv19 <- 
  ace_monthly_count_pre_cv19 %>% 
  model(mean = MEAN(n)) %>%
  interpolate(ace_monthly_count_pre_cv19)

ace_monthly_quantity_gp_pre_cv19 <- 
  ace_monthly_quantity_gp_pre_cv19%>% 
  group_by(regional_unit) %>% 
  model(mean = MEAN(n)) %>% 
  interpolate(ace_monthly_quantity_gp_pre_cv19)
...
```{r, ace-gp-practice-modelling}
plan(cluster)
system.time(ace_gp_cv_models <-
  ace_monthly_quantity_gp_pre_cv19 %>%
                  regionalCV(init = 48, step = 3))
plan(sequential)

ace_gp_cv_models %>%
  saveRDS("../data/ace_gp_cv_models.rds")

ace_gp_cv_models <- 
  read_rds("../data/ace_gp_cv_models.rds")
...
```{r ace-gp-practice-forecasts}
plan(cluster)
system.time(ace_gp_fcsts <-
              ace_monthly_quantity_gp_pre_cv19 %>%
...
ace_gp_fcsts %>%
  saveRDS("../data/ace_gp_fcsts.rds")

ace_gp_fcsts <- 
  read_rds("../data/ace_gp_fcsts.rds")
...
```{r, ace-gp-practice-discrepancies-abs}
best_model_gp <- 
  ace_gp_cv_models %>% 
  group_by(regional_unit) %>% 
  arrange(RMSE, MAE) %>% 
...
best_model_gp_discrep <- 
  ...
  inner_join(mutate(ace_gp_fcsts,
                    datename = yearmonth(datename)), 
             by = c(".model", "regional_unit")) %>%
  left_join(mutate(select(ace_monthly_quantity_gp, 
...
changes_pre_covid <- c("W95007", "W91020") # Significant changes to prescribing near the end of 2019

best_model_gp_discrep <- 
  best_model_gp_discrep %>% 
  filter(!regional_unit %in% changes_pre_covid)

best_model_gp_discrep %>% 
  saveRDS("../data/differences/ace_discreps.rds")
```

* `r length(changes_pre_covid)` GPs excluded as there was a significant change to their prescribing pre-covid
