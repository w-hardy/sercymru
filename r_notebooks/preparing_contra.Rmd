...
contra_monthly_quantity_gp <- 
  countRegional(contra, practice_id, datename, quantity) %>% 
...
contra_monthly_count_pre_cv19 <- 
  contra_monthly_count %>% 
  filter(datename < yearmonth(dmy("01-03-2020"))) 

contra_monthly_quantity_gp_pre_cv19 <- 
  contra_monthly_quantity_gp %>% 
  filter(datename < yearmonth(dmy("01-03-2020"))) 
...
contra_monthly_count_pre_cv19 <- 
  contra_monthly_count_pre_cv19 %>% 
  model(mean = MEAN(n)) %>%
  interpolate(contra_monthly_count_pre_cv19)

contra_monthly_quantity_gp_pre_cv19 <- 
  contra_monthly_quantity_gp_pre_cv19%>% 
  group_by(regional_unit) %>% 
  model(mean = MEAN(n)) %>% 
  interpolate(contra_monthly_quantity_gp_pre_cv19)
...
```{r, contra-gp-practice-modelling}
plan(cluster)
system.time(contra_gp_cv_models <-
  contra_monthly_quantity_gp_pre_cv19 %>%
                  regionalCV(init = 48, step = 3))
plan(sequential)

contra_gp_cv_models %>%
  saveRDS("../data/contra_gp_cv_models.rds")

contra_gp_cv_models <- 
  read_rds("../data/contra_gp_cv_models.rds")
...
```{r contra-gp-modelling-accuracy}
best_model_gp <- 
  contra_gp_cv_models %>% 
  group_by(regional_unit) %>% 
  arrange(RMSE, MAE) %>% 
...
```{r contra-gp-practice-forecasts}
plan(cluster)
system.time(contra_gp_fcsts <-
              contra_monthly_quantity_gp_pre_cv19 %>%
...
contra_gp_fcsts %>%
  saveRDS("../data/contra_gp_fcsts.rds")

contra_gp_fcsts <- 
  read_rds("../data/contra_gp_fcsts.rds")
...
```{r, contra-gp-practice-discrepancies}
best_model_gp <- 
  contra_gp_cv_models %>% 
  group_by(regional_unit) %>% 
  arrange(RMSE, MAE) %>% 
...
best_model_gp_discrep <- 
  ...
  inner_join(mutate(contra_gp_fcsts,
                    datename = yearmonth(datename)), 
             by = c(".model", "regional_unit")) %>%
  left_join(mutate(select(contra_monthly_quantity_gp, 
...
best_model_gp_discrep %>% 
  saveRDS("../data/differences/contra_discreps.rds")
